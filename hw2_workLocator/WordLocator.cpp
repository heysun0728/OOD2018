...
void WordLocator::error(){//print error string
        cout<<"ERROR: Invalid command"<<endl;
}
void WordLocator::load(string filename){
        ifstream file;
        string line;
        file.open(filename);
        if(!file){
...
void WordLocator::locate(string s,int pos){
        int i;
        int count=0;
        for(i=0;i<f.size();i++){
                if(s==f[i]) count++;
...
bool WordLocator::is_digits(string str)
{
...
int WordLocator::work(){//choose how to work and check paragraph
        if(arr[0]=="load"){
                if(arr.size()!=2) error();
                else load(arr[1]);
        }
